...
settings.post('/vsmtp', verify(), setting.vsmtp.default);
settings.post('/activate', verify(), setting.activate.default);
settings.post('/preview', verify(), setting.preview.default);

// --- WEBHOOKS --------------------------------------------------- //
const { SHOPIFY_API_SECRET } = process.env;
const webhookValidate = receiveWebhook({ secret: SHOPIFY_API_SECRET });
const webhooks = new Router({ prefix: '/webhook' });
const webhooks_list = ['orders/create', 'fulfillment_events/create', 'fulfillments/create', 'orders/fulfilled', 'orders/partially_fulfilled', 'app/uninstalled', 'gdpr/customers/redact', 'gdpr/shop/redact', 'gdpr/customers/data_request'];
